...
async def test_complete_flow():
    ...
    test_question = "Cuales son las mejores empresas de abogadoes en Chile para defenderse de las deudas?"
    ...
    try:
        ...
        for model, brands in analysis_result.items():
            ...
            if brands:
                for brand in brands:
                    print(f"    - {brand['brand']} (pos: {brand['position']}, sentiment: {brand['sentiment']}, links: {brand['link_count']})")
            else:
        ...
        record_id = await pocketbase_service.save_query(
            ...
            processed_responses=analysis_result
        )
